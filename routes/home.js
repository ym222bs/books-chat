...
router
  ...
  .post((req, res) => {
    ...
    GoogleBooks.search(searchField, { limit: 40 }, async (err, payload) => {
      err ? console.log(err) : console.log('payload sent')
      ...
      if (searchField !== '') {
        ...
        const categories = count(
          payload,
          (item) => item.categories || ['undefined']
        )

        let sorteradeCategories = Object.entries(categories).map((x) => ({
          category: x[0],
          count: x[1],
        }))
...
router.route('/add-favourite').post(async (req, res) => {
  const id = req.body.id

  const book = new Book()
  book.id = id
  book.userID = req.session.user
  book.username = req.session.username

  let match = await Book.find({ userID: req.session.user }, function (
    err,
    data
  ) {
